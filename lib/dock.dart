...
class DockManager extends StatelessWidget {
  ...
  const DockManager({Key? key, required this.builder}) : super(key: key);
...
class SplitterProvider extends InheritedWidget {
  ...
  @override
  bool updateShouldNotify(SplitterProvider oldWidget) => true;
...
mixin SplitterStateMixin<T extends Splitter> on State<T> {
  ...
  late ValueNotifier<String> layoutMask;

  double? _lastDividerPosition;
  late ValueNotifier<double?> dividerPosition;
  ...
  void initState() {
    ...
    layoutMask = ValueNotifier(_lastLayoutMask);
    layoutMask.addListener(_layoutMaskChanged);

    dividerPosition = ValueNotifier(null);
  ...
  List<Widget> _buildContent() {
    ...
    final split = _lastLayoutMask.split(MASK_SEPARATOR);
    ...
    for (var i = 0; i < widget.docks.length; i++) {
      ...
      final ratio = double.parse(split[i]);
...
class HorizontalSplitter extends Splitter {
  HorizontalSplitter({
    ...
    required List<Dock> docks,
    String? initialLayoutMask,
  }) : super(
          width: width,
          height: height,
          docks: docks,
...
class _DividerState extends State<Divider> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onPanStart: (details) {
        _delta = 0;
        _dividerPositionChanged(_delta, layoutMask.value, dividerPosition);
      },
      onPanUpdate: (details) {
        var changed;
        if (widget.stackedVertical) {
  ...
  void _dividerPositionChanged(
    ...
    // unbox the layout mask from string to easier format to read.
    final currentMask =
        layoutMask.split(MASK_SEPARATOR).map(double.parse).toList();
...
extension Precision on double {
  double toPrecision(int fractionDigits) {
    double mod = pow(10.0, fractionDigits.toDouble()).toDouble();
